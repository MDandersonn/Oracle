...
INSERT INTO REQ_ACCOUNTS(ID, NICKNAME, EMAIL)
       VALUES(SEQ_REQ_ACCOUNTS.NEXTVAL, 'jung', 'jung@gmail.com');
...
UPDATE REQ_ACCOUNTS
   SET ISALLOW = 'Y'
 WHERE REQDATE BETWEEN TO_DATE('20221128') AND TO_DATE('20221206')
   AND ISALLOW = 'N';
 ...
 */
 ...
 SELECT *
      FROM REQ_ACCOUNTS RA LEFT OUTER JOIN ACCOUNTS A ON RA.NICKNAME = A.NICKNAME AND RA.EMAIL = A.EMAIL
     WHERE RA.ISALLOW = 'Y' AND A.ID IS NULL;
...
UPDATE ACCOUNTS
 ...
 select * from ACCOUNTS_ACCESS_LOG;
...
SELECT *
  ...
  JOIN (SELECT ID, NICKNAME, EMAIL, 'passchange1234' AS CPASSWORD
          FROM ACCOUNTS
         WHERE NICKNAME = 'lee'
           AND PASSWORD = 'passchange') A2 ON A1.ID = A2.ID
 WHERE A1.NICKNAME = 'lee' AND A2.CPASSWORD LIKE '%' || A1.PASSWORD || '%';
